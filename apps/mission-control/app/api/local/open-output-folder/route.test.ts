...
function jsonRequest(body: unknown): NextRequest {
  return new NextRequest("http://localhost/api/local/open-output-folder", {
    method: "POST",
...
describe("open output folder route", () => {
  beforeEach(() => {
  ...
  it("rejects requests without an operator session, before spawning any process", async () => {
    ...
    const canOpenSpy = vi.spyOn(outputFolders, "canOpenLocalWindowsShell");
