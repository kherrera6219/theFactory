...
### Phase 6 UI Enhancements (2026-04-14)
...
- **Feature flag warning banners** (agents page): structured `role="alert"` block in Runtime Dependencies; warns when `consumer_running`, `protocol_ready`, `redis_ready`, or `db_ready` are false, and when `langgraph_enabled === false`.
...
### Phase 7 Extraction Enhancements (2026-04-14)
...
- **JS/TypeScript AST extractor** (`pod_worker/js_ast_extractor.py`): function/class/import extraction for JavaScript and TypeScript via `ast_grep` or `tree-sitter` fallback.
- **Java AST extractor** (`pod_worker/java_ast_extractor.py`): method/class/import extraction for Java source files.
...
6. `test_storage_unit.py` requires a live `postgres` host — excluded from the standard test run by `--ignore`; should be run in a Docker-compose integration environment.
7. `test_agent_base_unit.py` has a pre-existing broken import; excluded pending upstream fix.
