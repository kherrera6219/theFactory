...
| Mission Control Unit Suite | Vitest unit suite + `npm run test:coverage` on `app/lib/**` (60% line floor) |
| Pytest Backend Suite | Coverage gates: line ≥80%, branch ≥70%, mixed ≥80%, plus privilege-path floors |
...
| Live mission evidence | **PORT + fail-QC recorded; matrix remainder open** | Go S1-01 `mission-f8a5accf`, PyQt6 `mission-e42fd7e2`, Snake `mission-911a6b3f`. PORT-through-SOW `mission-dc0c8c4e` COMPLETE (`go.mod` + `main.go`). Failing QC `mission-8db1af71` VERIFIED / `MISSION_RUNTIME_QC_BLOCKED`. Still owed: failure injection, provider fallback, EDCP live-bus. |
| Audit & Quality Standards | **Hygiene green, not a release certificate** | `production_review_audit.py` is a static file/string check. Backend gates: line ≥80%, branch ≥70%, mixed ≥80%, plus sandbox/SOW/PORT/RQCA floors (PR #463). Mixed cannot hide a line score below 80% (`test_line_floor_fails_when_mixed_still_passes`). Makefile/CI keep `--line-threshold 80`. Mission Control measures `app/lib/**` via `npm run test:coverage`. Do not cite 23/23 as the release gate. |
