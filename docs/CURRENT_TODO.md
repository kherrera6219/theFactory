...
| **Pod-language runtime verification** | **17 of 19 languages verified live 2026-08-06.** Was 1 (Python). RQCA live execution now covers python, javascript, typescript, ruby, php, r, julia, ocaml, c, cpp, rust, go, zig, haskell, java, kotlin, scala — each verified by executing a hello-world through the real sandbox and asserting real stdout. **Not covered:** `matlab` and `mathematica` (no runnable image without a paid licence or network activation, and the sandbox is `--network=none`; GNU Octave is deliberately NOT used as a stand-in because passing it would assert something untrue about MATLAB compatibility) and `csharp` (`dotnet-script` absent from the SDK image, uninstallable offline). All three return DRY_RUN, which is honest and non-blocking — a listed-but-broken language returns FAIL and `RQCA_ENFORCEMENT_ENABLED` turns that into a blocked mission. **Caveat:** kotlin, scala, zig and ocaml have no official image, so they run in third-party ones; every report carries `base_image_official` so a surprising verdict can be traced to the toolchain rather than the artifact. Pinning those by digest is the open follow-up |
