...
Current checkpoint: Phase 8 is not complete and we should not advance to Phase 9 yet. Current Phase 8 fix batch removes order-dependent API gateway/protocol-bus failures: API gateway Prometheus collectors are re-import safe, gateway security tests patch the app-owning module instance, and protocol-bus key-warning coverage is deterministic. Full coverage now passes with 1,537 tests, 5 intentional skips, 81.50% total coverage, and the configured CI coverage threshold script passes. Outstanding Phase 8 work is to raise or explicitly defer stricter audit-policy coverage gaps (`mission_flow_v2/` 80.92% line / 59.64% branch vs 90% / 85%, `storage_agents.py` 76.47% line, `storage_artifacts.py` 63.79% line), inspect mock-self-assertion risk, verify fixture/schema realism, reconcile skip reasons with issue references or opt-in tier policy, and correct the audit plan's per-service directory expectation to match the repository's centralized `tests/services/test_*.py` convention.
