...
| **Pod-language runtime verification** | **17 of 19 languages verified live 2026-08-06.** Was 1 (Python). RQCA live execution now covers python, javascript, typescript, ruby, php, r, julia, ocaml, c, cpp, rust, go, zig, haskell, java, kotlin, scala — each verified by executing a hello-world through the real sandbox and asserting real stdout. **Not covered yet:** `matlab` and `mathematica` — the vendor runtimes need a paid licence or network activation and this product must run with **no external requirements**, so the route in is a licence-free *subset* runtime plus a report that states what it checked. Verified offline under the real sandbox flags 2026-08-06: `gnuoctave/octave:9.2.0` runs MATLAB and **rejects a realistic Python-fallback artifact (exit 1)** — ready to add; `mathicsorg/mathics:latest` runs Wolfram via `--quiet --file` but **exits 0 on Python source** (expression language: undefined symbols self-evaluate), so it needs a `failure_patterns` mechanism first or it would return a false PASS. Also uncovered: `csharp` (`dotnet-script` absent from the SDK image, uninstallable offline). All three return DRY_RUN, which is honest and non-blocking — a listed-but-broken language returns FAIL and `RQCA_ENFORCEMENT_ENABLED` turns that into a blocked mission. **Caveat:** kotlin, scala, zig and ocaml have no official image, so they run in third-party ones; every report carries `base_image_official` so a surprising verdict can be traced to the toolchain rather than the artifact. Pinning those by digest is the open follow-up |
