...
Current checkpoint: do not advance to Phase 9 yet. Current Phase 8 fix batch removed order-dependent API gateway/protocol-bus failures: API gateway Prometheus collectors are re-import safe, gateway security tests patch the app-owning module instance, and protocol-bus key-warning coverage is deterministic. Full coverage now passes with 1,537 tests, 5 intentional skips, 81.50% total coverage, and the configured CI coverage threshold script passes. Phase 8 still needs stricter audit-policy coverage work or explicit deferrals (`mission_flow_v2/` 80.92% line / 59.64% branch vs 90% / 85%, `storage_agents.py` 76.47% line, `storage_artifacts.py` 63.79% line), mock-self-assertion review, fixture/schema realism review, skip-policy reconciliation, and a decision to update the audit plan around the repo's centralized `tests/services/test_*.py` convention instead of forcing per-service test subdirectories.
