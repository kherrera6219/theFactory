...
| 09 | Refined-IR Specification | **Partial** | `schemas/rir.fn.schema.json`, `pod-worker/refined_ir.py` | Schema **Implemented** faithfully; the *producer* is templated (ADR row 7). §3 type system, §4 constraints, §5 side effects, §7 composition have no persisted representation. §8 equivalence execution is **Superseded** (ADR row 8) |
