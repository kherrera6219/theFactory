...
> **A1/A2/A3 status (2026-05-29): foundation + integrity verification ✅ DONE.**
...
> generated by `scripts/generate_prompt_manifest.py`) are implemented and tested (27 tests).
> **Remaining (incremental adoption):** wire `sign_payload`/`verify_payload` into the
> build-artifact / compliance-report / audit-bundle write+import paths, which requires the
> signing key in orchestrator app-state (same incremental-boundary approach as A5).
