...
- Gemini 3.1 Pro Preview is positioned for deepest reasoning and agentic coding workloads.
- Gemini 3 Flash Preview is positioned for low-latency operational and coding tasks.
- As of March 2, 2026, live `listModels` and `generateContent` tests confirm `gemini-3.1-pro-preview` and `gemini-3-flash-preview`.
...
| AGENT-03-BROKER | API Broker | Gemini | gemini-3-flash-preview | thinking level low; fallback `openai/gpt-5.2` |
...
| AGENT-09-HW | Hardware-Mapping Injector | Gemini | gemini-3.1-pro-preview | thinking level high; fallback `openai/gpt-5.2-pro` |
| AGENT-10-TESTER | System Integration Tester | Anthropic | claude-opus-4-6 | adaptive thinking |
| AGENT-11-DEPLOY | Deployment Agent | Gemini | gemini-3-flash-preview | thinking level medium; fallback `openai/gpt-5.2` |
...
| AGENT-30-PODD-MGR | Pod D Sub-Manager | Gemini | gemini-3.1-pro-preview | thinking level high; fallback `openai/gpt-5.2-pro` |
...
- Gemini: `GEMINI_THINKING_LEVEL=low|medium|high` (Gemini 3) and `GEMINI_THINKING_BUDGET=-1|0|N` (Gemini 2.5)
...
- Gemini pod agents: use general Gemini reasoning models (`gemini-3.1-pro-preview` / `gemini-3-flash-preview`), since Google does not expose a separate coding-only model family in Gemini API; coding performance is built into these general models.
