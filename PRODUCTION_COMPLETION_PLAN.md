...
- **Tests first / regression discipline.** New behaviour requires tests that would have failed against the pre-change code (use `git stash` or equivalent to prove it when practical). Keep global backend coverage ≥ 80% and existing per-module floors. UI: Vitest + Playwright critical paths.
